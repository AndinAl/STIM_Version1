...
def load_rows(transfer_dir: Path, glob_pattern: str, name_filter: str) -> list[dict]:
    rows: list[dict] = []
...
def main() -> None:
    ...
    parser.add_argument("--prefix", default="table_transfer_budget_sweep")
    args = parser.parse_args()

    transfer_dir = Path(args.transfer_dir)
    outdir = Path(args.outdir)
    outdir.mkdir(parents=True, exist_ok=True)
    rows = load_rows(transfer_dir, args.glob, args.name_filter)

    md_path = outdir / f"{args.prefix}.md"
    tex_path = outdir / f"{args.prefix}.tex"
