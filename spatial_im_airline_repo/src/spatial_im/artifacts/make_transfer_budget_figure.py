...
def load_transfer_grid(transfer_dir: Path) -> tuple[list[int], list[int], dict[str, dict[tuple[int, int], dict[str, float]]]]:
    rows: dict[str, dict[tuple[int, int], dict[str, float]]] = {regime: {} for regime in REGIMES}
    targets: set[int] = set()
    budgets: set[int] = set()

    paths = sorted(transfer_dir.glob("transfer_target*_budget*.json"))
    if not paths:
        raise FileNotFoundError(f"No transfer artifacts found in {transfer_dir}")
...
def main() -> None:
    ...
    out_path = outdir / "fig_transfer_budget_sweep.png"

    targets, budgets, rows = load_transfer_grid(transfer_dir)
